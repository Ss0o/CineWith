...
- 장르 탐색은 TMDB 장르 ID를 사용하며 액션·모험·애니메이션·코미디·드라마·판타지·공포·로맨스·SF·스릴러를 제공한다.
- 사이드바의 추천·현재 상영·개봉 예정·장르 목록은 TMDB의 20개 결과 페이지를 그대로 조회하며, 프론트엔드는 해당 페이지를 카드 그리드와 페이지 이동으로 표시한다.
