...
async function searchMovies() {
  ...
  errorMessage.value = ''
  try {
    const results = await dataService.movies.search(query.value.trim())
    if (request === searchVersion) { searchResults.value = results; searched.value = true }
  } catch (error) { if (request === searchVersion) errorMessage.value = error.message }
  finally { if (request === searchVersion) searching.value = false }
}

async function submitReview() {
  if (submitting.value) return
  if (!isMember.value) { openLoginModal(); return }
  errorMessage.value = ''
  if (!title.value.trim() || !content.value.trim()) { errorMessage.value = '제목과 본문을 입력해주세요.'; return }
...
<template>
  ...
  <div style="padding: 26px 32px 32px; display: flex; gap: 26px; max-width: 860px; margin: 0 auto; width: 100%">
    <div style="flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 18px">
      ...
      <p v-if="!isMember" class="meta">로그인한 회원만 리뷰를 작성할 수 있습니다.</p>
      <p v-if="movieLoading" role="status">선택한 영화를 불러오는 중입니다.</p>
      ...
      <div class="field">
        ...
        <div v-if="!selectedMovie" style="display: flex; gap: 8px">
          ...
          <button class="btn btn-secondary" :disabled="searching" @click="searchMovies">{{ searching ? '검색 중…' : '검색' }}</button>
        </div>
        <p v-if="!selectedMovie && searched && !searchResults.length" role="status">검색 결과가 없습니다.</p>
        ...
        <div v-if="selectedMovie" style="display: flex; align-items: center; gap: 12px; padding: 10px 12px; border: 1px solid var(--color-accent); border-radius: var(--radius-md); background: var(--color-surface)">
          <PosterThumb width="34px" height="48px" :label="selectedMovie.title" :poster-path="selectedMovie.posterPath" />
          <div style="flex: 1">
    ...
    <div style="width: 236px; flex: none; display: flex; flex-direction: column; gap: 16px">
      <div style="padding: 12px; border-radius: var(--radius-md); border: 1px solid var(--color-divider); font: 400 11.5px/1.7 var(--font-body); color: color-mix(in srgb, var(--color-text) 50%, transparent)">
        평점은 0.5점부터 5.0점까지 선택할 수 있습니다. 한 영화에는 리뷰를 하나만 작성할 수 있으며, 작성 후에도 제목·본문·평점을 수정할 수 있습니다.
      </div>
    </div>
  </div>
</template>
